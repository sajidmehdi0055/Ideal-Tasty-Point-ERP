...
## Independent review correction, round 2 (2026-09-18)

A focused re-review of the BLOCKER 2 fix returned **FAIL** again: the two-migration split was real, but node-pg-migrate's `up` command defaults `--single-transaction` to `true`, so a normal `npm run migrate` batch run still wrapped Migration A and Migration B in one outer transaction and rolled Migration A back along with Migration B on failure — the split existed but was inert under the actual authoritative command. Fixed: added `--no-single-transaction` to both `migrate` and `migrate:check` in `package.json` (the single authoritative place this setting is controlled), and rewrote the integration test suite to spawn the real `node-pg-migrate` CLI binary with the same flags (`tests/integration/helpers/migrate-cli.ts`) instead of using the programmatic `runner()` API with artificially separated invocations. The full recovery sequence was re-verified, manually and by an automated test, using this real command. See ADR-0007 D-05 (round 2 update note).
...
| Safety refinement: no guessed unit_type for unmatched legacy base_uom, with an executable recovery path | Split into Migration A (UOM/Brand) and Migration B (backfill + safety-halt + Pack Variant), **plus** `--no-single-transaction` on `migrate`/`migrate:check` (round-2 fix — the split alone was inert against node-pg-migrate's single-transaction-by-default batch behavior); a Migration B halt via the real authoritative command leaves Migration A committed, so the documented recovery (classify the UOM explicitly, re-run the same command) actually works; proven manually and by an integration test spawning the real CLI binary through the complete recovery sequence |
...
- The `base_uom` backfill never guesses a `unit_type` for an unmatched legacy value; it halts and rolls back, and — split across two migrations with `--no-single-transaction` explicitly set (corrected per BLOCKER 2, both rounds) — the halt is actually recoverable through the real authoritative command: `uom_master` survives, an operator classifies the missing value, and re-running the same `npm run migrate` succeeds.
