...
- Agent count is task-dependent, not fixed: scale specialists to what a task genuinely needs (roughly 1–2 for a small task, 3–6 medium, 7–10 large, 10–15+ only for genuinely complex cross-module work), with clear non-overlapping roles and no concurrent edits to the same file/subsystem without coordination. See docs/engineering/MULTI-AGENT-OPERATING-MODEL.md for the full scaling guide, extended specialist role catalog, escalation policy, the Agent Execution Report format, and the project's execution model (Claude Code only, while Codex and Google Antigravity are paused — see below).
- **Codex and Google Antigravity paused (owner instruction, 2026-09-25):** until the owner reactivates them, all implementation and review happen inside Claude Code. The Manager still never self-certifies: a dedicated QA/Testing subagent, separate from and with no prior involvement in the implementing subagent's work, actually runs the required tests/checks and independently verifies the result before the Manager reports completion. This is a same-provider substitute for a genuinely external reviewer, not equivalent to it — the Manager states this limitation plainly in every completion report so the owner can weigh it.
- **Token/cost discipline:** do not spend effort on work the task doesn't need — no redundant re-reading of files already read this task, no speculative exploration beyond the bounded scope, no busywork to appear thorough, no gold-plating past the approved acceptance criteria. Keep subagent counts to what the task genuinely requires (see the scaling table above) and stop once the acceptance criteria are met.
- **When genuinely unsure — ambiguous instruction, conflicting documentation, or a decision only the owner can make — stop and ask the owner directly** rather than guessing or proceeding on an assumption.
